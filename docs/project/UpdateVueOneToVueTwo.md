...
在持续推进的项目中，如何将大家不断提交的`git commit`完整安全的合并到现有的代码中，是件很麻烦的事情。

过程中，难以避免漏合或合错的情况。但请不要害怕，如果不去尝试，将永远无法将项目升级和推进。

我们需要尽可能减少该错误，即使错了，我们还可以通过测试进行纠正，所以请细心大胆的开始吧！

`git cherry`将成为迁移项目的重要工具，请熟知它！

首先：将原始仓库添加到该仓库的远程地址中。

将原始仓库的主分支pull到本地：

```git
git checkout 原型仓库名/主分支 -f
```

:::tip
请使用`-f`，强制将非关联性的仓库的获取到本地。
:::

有了本地仓库，就可以通过`cherry-pick`将不同阶段的提交有选择性合并到该仓库来。

此时需要注意的是：

1. 不要将`develop to develop`进行合并，只合并具体内容的提交。

2. 合并冲突是必然的，提交的commit内容为：“cherry 合并过来的那条commit信息”

3. 尽量将每天的最新的提交合并到该仓库里面来！
...
在没有单元测试的情况下，想详尽的进行测试是一项近乎无法完成的任务。

即使开发者对此处了如指掌，何况更多的开发者也无法掌握，因此不要做详尽测试，做频繁出错的测试。

同样，一开始也请不要进行整体测试，测完一处后因为逻辑存在而去测试另一处。

这不仅会打断你当前的单元测试，也会徒增你的思考和阻碍你的效率。

单元猜测不应过多的注意细节，细节可以通过后续的快速补救，而应关注整体流程，如果一个流程是通的，那么可以说是通过“安全测试”。
...
:::warning
请先忽略数据逻辑上的测试，也请不要注意细节。
:::

### 受理办件流程

受理办件流程是整个“受理系统”的核心内容，所有的功能都是围绕着受理系统进行运作的。

因此，将受理流程测试通过是重中之重，应立即提上日程。
